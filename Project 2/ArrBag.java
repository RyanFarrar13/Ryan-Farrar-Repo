...
ArrBag<T> union = new ArrBag<T>(this.size() + other.size());
         ...
         for( int j = 0; j < this.size(); j++){
			 union.add(this.get(j));
			 
		 }
		 for(int a = 0; a < other.size(); a++){
			 if(!union.contains(other.get(a))){
				 union.add(other.get(a));
			 }
		 }
	...
	{
		ArrBag<T> intersection;
		if(this.size() < other.size()){
		intersection = new ArrBag<T>(this.size());	
		}
		else{
			intersection = new ArrBag<T>(other.size());
		}
		for(int i=0;i<this.size();i++){
